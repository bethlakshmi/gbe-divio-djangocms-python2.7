...
class CachedLoader(list):
    ...
    def __init__(self, loaders):
        ...
        methods = ("append", "extend", "insert", "remove", "pop", "index", "count")
...
class Form(forms.BaseForm):
    ...
    def to_settings(self, data, settings):
        ...
        if not settings["CACHE_URL"]:
            settings["CACHE_URL"] = "locmem://"
            warnings.warn(
                "no cache configured. Falling back to CACHE_URL={0}".format(settings["CACHE_URL"]),
                RuntimeWarning,
            )
        ...
        if not env("DISABLE_GZIP"):
            settings["MIDDLEWARE"].insert(0, "django.middleware.gzip.GZipMiddleware")
    ...
    def domain_settings(self, data, settings, env):
        ...
        if not domains and domain:
            domain_aliases = [d.strip() for d in env("DOMAIN_ALIASES", "").split(",") if d.strip()]
            domain_redirects = [
                d.strip() for d in env("DOMAIN_REDIRECTS", "").split(",") if d.strip()
            ]
            domains = {
                env("SITE_ID", 1): {
                    "name": env("SITE_NAME", ""),
    ...
    def security_settings(self, data, settings, env):
        ...
        s["SESSION_COOKIE_AGE"] = env(
            "SESSION_COOKIE_AGE", data.get("session_timeout") or 60 * 60 * 24 * 7 * 2
        )
    ...
    def server_settings(self, settings, env):
        ...
        settings["STATICFILES_DEFAULT_MAX_AGE"] = env("STATICFILES_DEFAULT_MAX_AGE", 300)
    ...
    def i18n_settings(self, data, settings, env):
        ...
        def language_codes_to_tuple(codes):
            return [(code, settings["ALL_LANGUAGES_DICT"][code]) for code in codes]
